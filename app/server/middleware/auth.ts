...
export async function requireAdminAuth({
  ...
  try {
    if (isGemXDevMode()) {
      ...
      shopify.admin.graphql = graphqlClient(
        offlineSession,
      ) as AdminContext["admin"]["graphql"];
      serviceProvider.register<AdminContext>("ShopifyContext", shopify);
      return shopify;
    }

    const shopify: AdminContext = await authenticate.admin(request);
  ...
  return async (
    ...
    try {
      ...
      const err =
        error instanceof Error
          ? error
          : new Error("Unknown error in withAdminAuth");
